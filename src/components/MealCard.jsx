...
export default function MealCard({meal , id , votingPeriod}) {
    ...
    const handleHideMeal = async (e) => {
            ...
            if (mealData){
              if (mealData.votes == 0){
                ...
                toast.error("You can't hide a meal once it has been voted for. Voting will reset tomorrow.")
              }
            }
  ...
  return (
    <>
    {votingPeriod === true && !mealVotedFor &&
    ...
    { ( votingPeriod === false || (votingPeriod === true && mealVotedFor) ) &&
      <div key={meal.id} onClick={() => toast.error("Voting is closed. You can vote again tomorrow!")}  className='meal-card grid grid-cols-12 justify-start flex-col m-3 bg-white  rounded-md shadow-md text-left'>
        <div className="w-32 aspect-square rounded col-span-2">
          <img src={meal.img} className="object-cover  rounded-md h-[100%]" />
        </div>
        <div className='col-span-10 ml-24 md:ml-20 xl:ml-10 span-8 self-center'>
        ...
        <p className=' text-3xl font-bold text-gray-400'>{meal.title}</p>
        <p className=' text-sm font-medium break-words w-[100%]  text-gray-400'>{meal.description}</p>
        </div>
        <div className='col-span-2 text-center'>
        {/* {admin && <button  className='bg-gray-600 hover:bg-red-700 transition-all ease-in-out duration-100 rounded py-2 px-2 m-2'>H</button>} */}
        </div>
